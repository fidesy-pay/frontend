...
const Transfer: React.FC = () => {
    ...
    const { loading: clientsLoading, error: clientsError } = useQuery(ClientsQuery, {
        ...
        <div className="mt-12 max-w-md w-full px-3 md:p-0">
            ...
            <div className="mt-2 md:mt-6 ring-2 ring-gray-100 custom-shadow py-9 px-6 rounded-3xl">
                ...
                <div className="mt-6 mb-4">
                    ...
                    {!user &&
                        <input type="text" id="address" value={receiverAddress} onChange={(e) => {
                            setReceiverAddress(e.target.value);
                            setFetchingUsers(true); // Start fetching users when typing
                        }} className="border border-gray-300 rounded-md p-2 w-full" />
                    }
                    {user && <Client username={user.username} photo_url={user.photo_url}/>}
                    ...
                    {userList.length > 0 && (
                        <div className="absolute bg-white w-full max-w-md mt-1 rounded-md border border-gray-300 z-10">
                            {userList.map((u: any) => (
                                <div key={u.id} className="p-2 cursor-pointer hover:bg-gray-100" onClick={() => {
                                    setUser(u);
                ...
                <div className="mb-4">
                    <label className="block text-gray-700 text-sm font-bold mb-2 text-start" htmlFor="gas_limit">
                        Gas limit (optional)
                    </label>
                    <input type="number" id="gas_limit" value={gasLimit} onChange={(e) => setGasLimit(e.target.value)} className="border border-gray-300 rounded-md p-2 w-full" />
                </div>

                {feeLoading && <p>Loading...</p>}
                {feeError && <p className="text-center text-red-500 mt-4">Error fetching fee</p>}
                {fee && (
                   <div>
                       ...
                       <div className="mt-5 flex items-center">
                           <label className="block text-gray-700 text-sm font-bold text-start">Amount with fee:</label>
                           <div className="mx-2 flex items-center">
                               <span className="text-sm text-gray-600">{transferAmount != "" ? fee.token_amount+parseFloat(transferAmount) : fee.token_amount}</span>
                               <div className="mx-3">
                                   {selectedChain.symbol === "MATIC" ?
                                       <img src={polygonMaticLogoUrl} alt="Matic Logo" className="h-8 w-8" /> :
                                       <img src={ethereumLogoUrl} alt="Ethereum Logo" className="h-8 w-8" />
                ...
                {transferError && <p className="mt-5 text-center text-gray-600">{ transferError }</p>}
            </div>
        </div>
    );
};
