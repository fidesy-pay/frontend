...
const paymentNotAvailableStatuses = ["SUCCESS", "FAILED", "MANUAL_CONTROL"];

export const PolygonMatic: PaymentOption = {
  name: "Polygon Matic",
  chain: "polygon",
  token: "matic-network",
  symbol: "MATIC",
  image: polygonMaticLogoUrl,
};
export const ArbitrumEthereum: PaymentOption = {
...
export const Invoice: React.FC = () => {
  ...
  const [selectedPaymentOption, setSelectedPaymentOption] =
    useState<PaymentOption | null>(PolygonMatic);
  ...
  const fetchInvoice = async () => {
    try {
      const response = await fetch(
        `http://facade.pay.fidesy.tech/api/invoice/${invoice_id}`,
      );
      if (!response.ok) {
          ...
          <div className="p-4">
            <div className="flex flex-col space-y-3">
              <div className="flex items-center justify-between border-b border-gray-300 py-2">
                <span className="text-sm font-medium text-gray-600 mr-5">
                  ID:
                </span>
              ...
              <div className="flex items-center justify-between border-b border-gray-300 py-2">
                ...
                <span className="text-sm text-gray-700">
                  ${invoice.usd_amount.toFixed(2)} <br />{" "}
                  {invoice.token_amount != undefined &&
                    invoice.token_amount.toFixed(18)}{" "}
                  {invoice.token}
                </span>
              </div>
              <div className="flex items-center justify-between border-b border-gray-300 py-2">
